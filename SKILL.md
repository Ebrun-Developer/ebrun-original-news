...
1. **唯一合法入口**：无论任何环境、任何场景，必须通过 `src/index.ts` 作为执行此技能的唯一入口，禁止绕过主入口直接调用任何子组件
2. **主入口已内置所有能力**：`src/index.ts` 已完整实现频道匹配、API请求、自动降级（Python/Shell脚本）、版本更新检查、结果格式化、异常处理的所有逻辑，外部不需要额外处理任何环节
3. **明确禁止的操作**：
