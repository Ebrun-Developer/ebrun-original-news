...
class EbrunSkill {
    constructor() {
        ...
        this.runStartTime = 0;
        this.RUN_TIMEOUT = 30000; // 30秒超时保护
    }
    loadConfig() {
        if (this.config)
            return this.config;
    ...
    matchChannel(userInput) {
        ...
        for (const [channelName, channelData] of Object.entries(config.channels)) {
            ...
            if (channelKeywords.some(keyword => input.includes(keyword) ||
    ...
    async fetchNews(apiUrl, limit, context) {
        if (!isSafeUrl(apiUrl)) {
    ...
    formatOutput(articles, channel, subChannel, isFallback, userInput) {
        if (articles.length === 0)
            return '📭 当前频道暂无最新文章，请稍后再试。';
     ...
     */
    async checkUpdateInBackground(ctx) {
    ...
    async _runInternal(ctx) {
        ...
        try {
            ...
            if (!path)
                throw new Error(`配置缺失: 无法找到 ${channel} 频道对应的路径`);
            const apiUrl = `${config.base_url}${path}`;
            if (!ctx.say)
                console.log(`🔍 正在获取 ${channel}${subChannel === '最新' ? '' : ' - ' + subChannel} 最新动态...\n`);
            // 优化：并行执行新闻获取和后台更新检查
...
export const run = (ctx) => skill.run(ctx);
export const main = (args = []) => skill.run({ args });
// 命令行执行入口
